call_aggregator(Name, [X], L):-
	setof([X], call(lat:Name, X), L).
call_aggregator(Name, [X,Y], L):-
	setof([X,Y], call(lat:Name, X,Y), L).
call_aggregator(Name, [X,Y,Z], L):-
	setof([X,Y,Z], call(lat:Name, X,Y,Z), L).
call_aggregator(Name, [W,X,Y,Z], L):-
	setof([W,X,Y,Z], call(lat:Name, W,X,Y,Z), L).
call_aggregator(Name, [V,W,X,Y,Z], L):-
	setof([V,W,X,Y,Z], call(lat:Name, V,W,X,Y,Z), L).
call_aggregator(Name, [U,V,W,X,Y,Z], L):-
